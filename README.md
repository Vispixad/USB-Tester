...
Это меню измерения емкости батарей и прочего, в самом вверху есть индикатор ON/OFF который показывает включена ли запись данных о емкости в ячейку. Ниже показано номер ячейку куда будет идти запись, время записи, емкость в Ah и Wh, также напряжение и предположительное КПД батареи. Красным показаны Ah только пересчитаны с выбранными вами напряжением и КПД.

> [!CAUTION]
> Сохранение идет только после остановки записи удержанием, поэтому если вы выключите устройство до остановки данные не будут сохранены. Если делать сохранение через какое то время 0.5-1s, память проживет гораздо меньше из-за более частого количества записей.

Что касается навигации, она чуть отличается:
- **нажатие (средняя кнопка)** - подсветит один из параметров (номер ячейки, напряжения, КПД).
     - **нажатие (кнопка вперед)** - если после нажатие средней кнопки нажать кнопку вперед, мы сможем выбирать по очереди один из параметров (выбранный будет подсвечен синим прямоугольником).
