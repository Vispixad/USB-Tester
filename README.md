...
## INA3221 (АЦП)
Следущее что мы можем увидеть при включении тестера после дисплея это основные данные, (V, A, W) для этой цели я преобрел АЦП **INA3221**. Это цифровой трехканальный датчик тока и напряжения с шиной I2C, нам будут нужны несколько каналов так как есть
