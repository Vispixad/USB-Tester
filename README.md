...
> [!IMPORTANT]
> Стоит также отметить что в графике **CC1/CC2**, если у вас не подключено устройство по **USB-C** тестер вам об этом сообщит сообщением, пример показан ниже:
> 
...
Последнее дочерное меню это - меню приложений или **Application**, здесь в прямоугольниках идут разветвления на другие под меню о которых написано ниже.

Также внизу показывается **время работы устройства**, что может быть полезным в некоторых случаях. По поводу управления, **нажатием кнопки вперед** мы будем выбирать разные подкатегории (выбранная отмечается синим), **нажатием кнопки назад** мы перейдем на предыдущее меню, **нажатие средней кнопки** сделает переход в подменю или подкатегорию.
