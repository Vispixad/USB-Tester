void rotatePointRec(int &x, int &y, int angle, int centerX, int centerY) {
...
void drawTriangleRec1(int posX, int posY, int angle) {
  ...
  rotatePointRec(temp_x0, temp_y0, angle, centerX, centerY);
  rotatePointRec(temp_x1, temp_y1, angle, centerX, centerY);
  rotatePointRec(temp_x2, temp_y2, angle, centerX, centerY);
...
void drawTriangleRec2(int posX, int posY, int angle) {
  uint16_t triangleColor = tft.color565(0, 227, 91);
  ...
  // Rotate each point in place
  rotatePointRec(temp_x0_2, temp_y0_2, angle, centerX_2, centerY_2);
  rotatePointRec(temp_x1_2, temp_y1_2, angle, centerX_2, centerY_2);
  rotatePointRec(temp_x2_2, temp_y2_2, angle, centerX_2, centerY_2);
...
void drawTextGraphRec() {
  tft.setRotation(3);
  tft.setFreeFont(&PTSans_Bold9pt7b);
  uint16_t textColor1 = tft.color565(255, 239, 0);
  uint16_t textColor2 = tft.color565(0, 227, 91);
...
void drawPauseGraphRec() {
  static unsigned long lastButtonRec = 0;
  static const unsigned long singleButtonRec = 150;
  unsigned long currentRec = millis();
  if (currentRec - lastButtonRec >= singleButtonRec) {
    if (digitalRead(BUTTON_DOWN) == LOW) {
      /*if (!tolldown) {*/
      lastButtonRec = currentRec;
