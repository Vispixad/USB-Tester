/*void rotatePoint(int &x, int &y, int angle, int centerX, int centerY) {
  ...
  void drawRotatedTriangle(int posX, int posY, int angle) {
  uint16_t triangleColor = tft.color565(2, 172, 230);
  ...
  rotatePoint(x1, y1, angle, centerX, centerY);
  rotatePoint(x2, y2, angle, centerX, centerY);
...
void drawRectQC3() {
  ...
  tft.fillRoundRect(60, 50, 35, 210, 9, tft.color565(80, 80, 80));
  tft.fillRoundRect(60, 50, 35, 70, 9, tft.color565(2, 91, 232));
...
void drawTextQC3() {
  ...
  tft.drawString("Qualcomm QC3.0", 82, 5);
...
void buttonQC3() {
  unsigned long lastqc3Down = 0;
  const unsigned long qc3Down = 100;
  if (digitalRead(BUTTON_DOWN) == LOW && millis() - lastqc3Down > qc3Down) {
    if (!QC3down) {
      QC3down = true;
      stopedQC3 = true;
